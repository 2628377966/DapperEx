...
namespace DapperEx
{
    public abstract class SqlQuery
    {
        protected SqlQuery()
        {
            ...
            this._outParam = new DynamicParameters();
        }
        ...
        protected IList<string> ExcColums;//加入SQL执行的列
        //动态参数类缓存
        ...
        protected DynamicParameters _outParam;
            ...
            get
            {
                if (this._Param != null && this._Param.Count > 0)
                {
                    ...
                    */
                    ...
                    #region 根据参数及参数类型生成缓存键
                    StringBuilder sb = new StringBuilder();
                    var parm = this.ParamValues.OrderBy(e => e.Key);
                    foreach (var item in parm)
                    {
                        sb.Append(item.Key);
                        if (item.Value != null)
                        {
                            sb.Append(item.Value.GetType().FullName);
                        }
                    }
                    MD5CryptoServiceProvider md5 = new MD5CryptoServiceProvider();
                    byte[] encryptedBytes = md5.ComputeHash(Encoding.ASCII.GetBytes(sb.ToString()));
                    sb.Clear();
        ...
        internal virtual string OutParamName
        {
            get
            {
                return string.Format("{0}NewInsertID", ParamPrefix);
            }
        }
        internal virtual DynamicParameters IntIDOutParam
        {
            get
            {
                return _outParam;
        ...
        /// 插入语句SQL
        /// </summary>
        internal virtual string InsertSqlRetIDENTITY
        {
            get
            {
                string[] sql = { "@@IDENTITY", "SCOPE_IDENTITY()" };
                string retSQL = string.Format(";select {0}={1}", OutParamName, ParamPrefix == "?" ? sql[1] : sql[0]);
                _outParam.Add(OutParamName, null, System.Data.DbType.Int32, System.Data.ParameterDirection.Output, null);
                return InsertSql + retSQL;
            }
        }
        /// <summary>
        /// 删除SQL
        ...
        /// 修改SQL(多主键的)
        /// </summary>
        internal virtual string UpdateSqlKeys
        {
            get
            {
                StringBuilder sql = new StringBuilder();
                ...
                if (string.IsNullOrEmpty(WhereSql))//没有where条件的情况
                {
                    var pList = Common.GetPrimarys(this._ModelDes);
                    for (int i = 0; i < pList.Count; i++)
                    {
                        ...
                        {
                            sql.Append(string.Format(" AND {0}={1}", p.Column, ParamPrefix + p.Field));
                        }
                    }
                }
                else
    ...
    public class SqlQuery<T> : SqlQuery where T : class
    {
        ...
        /// 创建
        /// </summary>
        /// <param name="ParamPrefix">前缀</param>
        /// <param name="DbType">数据库类型</param>
        /// <returns></returns>
        public static SqlQuery<T> Builder(string ParamPrefix, DBType DbType)
        {
            var result = new SqlQuery<T>();
            result.ParamPrefix = ParamPrefix;
        ...
        /// <returns></returns>
        public SqlQuery<T> AndWhere(Expression<Func<T, object>> expr, OperationMethod operation, object value=null)
        {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public SqlQuery<T> OrWhere(Expression<Func<T, object>> expr, OperationMethod operation, object value=null)
        {
